...
export class DesertRaceComponent implements OnInit {
  ...
  move(direccion: string, number: number) {
    if (direccion == "left" && this.auto.x >= 0) { // Player holding left
      this.auto.x -= this.auto.speed * number;
    }
    if (direccion == "right" && this.auto.x <= 195) { // Player holding right
      this.auto.x += this.auto.speed * number;
    }
    ...
    this.autosEnemigos.forEach(auto => {
      if (this.auto.x <= (auto.x + 22) && auto.x <= (this.auto.x + 22)
        && this.auto.y <= (auto.y + 22) && auto.y <= (this.auto.y + 22)) {
        ++this.crash;
        this.continue = false;
      }
    })
  ...
  render() {
    if (this.ctx) {
      ...
      this.ctx.fillText("Crashes: " + this.crash, 32, 32);
    }
  };
  ...
  generacionEnemigos() {
    ...
    for (let i = this.autosEnemigos.length; i < (this.puntaje / 200); i++) {
      if (this.canvas) this.autosEnemigos.push(new Car((Math.random() * (this.canvas.width - 24)), -100, Math.floor((Math.random() * 5))));
    }
    console.log(this.auto.x);
  }
